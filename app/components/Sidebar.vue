<template>
    <aside class="flex flex-col w-64 bg-emerald-50/30">
        ...
        <nav class="flex flex-col p-3 flex-1">
            <!-- Profile nav item -->
            <div v-if="profileStore.pending" class="flex items-center h-12 px-3 gap-4">
                <div class="w-6.5 h-6.5 rounded-xl bg-neutral-200 animate-pulse shrink-0" />
                <div class="h-3 w-24 rounded-full bg-neutral-200 animate-pulse" />
            </div>
            <SidebarNavItem
                v-else
                href="/profile"
